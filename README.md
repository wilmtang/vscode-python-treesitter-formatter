...
- **AST-aware on-type indentation**: as you type, the caret is auto-indented from the live parse tree — **Enter** indents the new line (block openers like `def`/`if`/`for`/`class`, hanging vs. visual bracket continuation, dedent after `return`/`raise`/`pass`/`break`/`continue`); typing **`:`** re-aligns a dedenting clause header (`else`/`elif`/`except`/`finally`/`case`) to its opener; and a closing **`)` `]` `}`** that starts a line snaps to the line that opened it. Reading the tree instead of regex rules lets it tell a block `:` from a `dict`/slice/`lambda` `:`, and it falls back silently to the editor's own indent whenever it can't confidently decide.
...
- `treesitterFormatter.indentOnType` (default `true`) — auto-indent as you type, computing the caret's indent from the syntax tree (see **AST-aware on-type indentation** above). Relies on `editor.formatOnType`, which this extension turns on by default for Python.
